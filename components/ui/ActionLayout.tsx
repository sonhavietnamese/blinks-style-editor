...
import { ActionCheckboxGroup } from './inputs/ActionCheckboxGroup'
import { ActionTextArea } from './inputs/ActionTextArea'
...
const Linkable = ({ url, className, children }: { url?: string | null; className?: string; children: ReactNode | ReactNode[] }) =>
  url ? (
    ...
    return (
      <div className={className}>
        <Snackbar variant='error'>
          <p>
            This Action or it&apos;s origin has been flagged as an unsafe action, & has been blocked. If you believe this action has been blocked in
            error, please{' '}
            <a href='https://discord.gg/saydialect' className='cursor-pointer underline' target='_blank' rel='noopener noreferrer'>
              submit an issue
            </a>
            .{!ignorable && ' Your action provider blocks execution of this action.'}
          </p>
          {ignorable && onSkip && (
            <button className='mt-3 font-semibold transition-colors hover:text-text-error-hover motion-reduce:transition-none' onClick={onSkip}>
...
export const ActionLayout = ({
  ...
  return (
    <div id='blink-preview' className={clsx('blink', stylePresetClassMap[stylePreset])}>
      <div className='w-full cursor-default overflow-hidden rounded-2xl border border-stroke-primary bg-bg-primary shadow-action'>
        {image && (
          <Linkable url={websiteUrl} className='block max-h-[100cqw] overflow-y-hidden px-5 pt-5'>
            <img className={clsx('aspect-auto w-full rounded-xl object-cover object-center')} src={image} alt='action-image' />
          </Linkable>
        )}
        <div className='flex flex-col p-5'>
          <div className='mb-2 flex items-center gap-2'>
            ...
            {websiteText && !websiteUrl && <span className='inline-flex items-center truncate text-subtext text-text-link'>{websiteText}</span>}
            <a href='https://docs.dialect.to/documentation/actions/security' target='_blank' rel='noopener noreferrer' className='flex items-center'>
              ...
              {type === 'trusted' && <Badge variant='default' icon={<InfoShieldIcon width={13} height={13} />} />}
              {type === 'unknown' && <Badge variant='warning' icon={<InfoShieldIcon width={13} height={13} />} />}
            </a>
          </div>
          <span className='mb-0.5 text-text font-semibold text-text-primary'>{title}</span>
          <span className='mb-4 whitespace-pre-wrap text-subtext text-text-secondary'>{description}</span>
          {disclaimer && (
            <DisclaimerBlock
              className='mb-4'
              ...
              onSkip={disclaimer.type === DisclaimerType.BLOCKED ? disclaimer.onSkip : undefined}
            />
          )}
          <ActionContent form={form} inputs={inputs} buttons={buttons} />
          {success && <span className='mt-4 flex justify-center text-subtext text-text-success'>{success}</span>}
          {error && !success && <span className='mt-4 flex justify-center text-subtext text-text-error'>{error}</span>}
        </div>
      </div>
    </div>
  )
...
const ActionContent = ({ form, inputs, buttons }: Pick<LayoutProps, 'form' | 'buttons' | 'inputs'>) => {
  if (form) {
...
const buildDefaultFormValues = (inputs: InputProps[]): Record<string, string | string[]> => {
  return Object.fromEntries(
    inputs
      .map((i) => {
        if (i.type === 'checkbox') {
          return [i.name, i.options?.filter((o: any) => o.selected).map((o: any) => o.value)]
        }
        return i.type === 'radio' ? [i.name, i.options?.find((o: any) => o.selected)?.value] : null
      })
      .filter((i) => !!i),
  )
}
...
const ActionForm = ({ form }: Required<Pick<LayoutProps, 'form'>>) => {
  const [values, setValues] = useState<Record<string, string | string[]>>(buildDefaultFormValues(form.inputs))
  const [validity, setValidity] = useState<Record<string, boolean>>(Object.fromEntries(form.inputs.map((i) => [i.name, false])))
  ...
  return (
    <div className='flex flex-col gap-3'>
      ...
      <ActionButton {...form.button} onClick={() => form.button.onClick(values)} disabled={form.button.disabled || disabled} />
    </div>
  )
}
